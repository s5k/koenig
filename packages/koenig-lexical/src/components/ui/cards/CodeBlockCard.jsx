    ...
    return (
        <div className="not-kg-prose min-h-[170px]">
            ...
            <input
                ...
                className={`z-999 absolute right-1.5 top-1.5 w-1/5 rounded border border-grey-300 px-2 py-1 font-sans text-[13px] leading-4 text-grey-900 transition-opacity focus-visible:outline-none dark:border-grey-900 dark:text-grey-400 ${showLanguage ? 'opacity-100' : 'opacity-0'}`}
...
export function CodeBlock({code, darkMode, language}) {
    const preClass = darkMode
        ? `rounded border border-grey-950 bg-grey-950 px-2 py-[6px] font-mono text-[16px] leading-9 text-grey-400 whitespace-pre-wrap`
        : `rounded border border-grey-200 bg-grey-100 px-2 py-[6px] font-mono text-[16px] leading-9 text-grey-900 whitespace-pre-wrap`;
    return (
